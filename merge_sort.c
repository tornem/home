...
void Merge(int* array, int left_index, int right_index, int arr_size)  // left_index < right_index < arr_size
{
    ...
    int* left_arr = malloc(left_size * sizeof(left_size) + 1);  // + 1 - место для маркеров максимального значения
    int* right_arr = malloc(right_size * sizeof(right_size) + 1); 
